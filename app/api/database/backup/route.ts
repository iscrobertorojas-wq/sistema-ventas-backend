...
export const GET = withAuth(async function GET() {
    try {
        ...
        for (const tableName of tableNames) {
            ...
            // Fix generic foreign key names (like fk_1, fk_2) generated by some DBs like TiDB
            // Make them globally unique by prefixing with the table name to avoid Error 1022 on restore
            createSql = createSql.replace(/CONSTRAINT `fk_(\d+)`/g, `CONSTRAINT \`fk_${tableName}_$1\``);
            createSql = createSql.replace(/KEY `fk_(\d+)`/g, `KEY \`fk_${tableName}_$1\``);
